...
def test_stub_generation_with_private_members(sample_project):
    ...
    cli.generate(input_path=str(main_file), output_path=str(stub_file), include_private=True)
...
def test_stub_generation_with_import_sorting(sample_project):
    ...
    cli.generate(input_path=str(main_file), output_path=str(stub_file), sort_imports=True)
...
def test_command_line_interface_integration(sample_project):
    ...
    try:
        ...
        result = subprocess.run(
            ["python", "-m", "twat_coding.pystubnik.cli", "generate", str(main_file), "--output_path", str(stub_file)],
            capture_output=True,
            text=True,
            check=True,
        )
